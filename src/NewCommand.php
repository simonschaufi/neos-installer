...
namespace Neos\Installer\Console;
...
class NewCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ->setName('new')
            ->setDescription('Create a new Neos project')
            ->addArgument('name', InputArgument::REQUIRED)
            ->addOption('release', null, InputOption::VALUE_OPTIONAL, 'Installs a specific Neos release')
    ...
    {
        // Logo from \Neos\CliSetup\Command\WelcomeCommandController
        $output->write(<<<EOT
            <fg=blue>
                ....######          .######
                .....#######      ...######
        ...
        $commands = [
            $composer." create-project neos/neos-base-distribution \"$directory\" \"$release\"",
        ];
        ...
        if (($process = $this->runCommands($commands, $input, $output))->isSuccessful()) {
            ...
            $output->writeln(PHP_EOL . '<comment>Neos project is ready! Build something amazing!</comment>');
        }
    ...
    /**
     * Create a Git repository and commit the base Neos skeleton
     */
    protected function createRepository(string $directory, InputInterface $input, OutputInterface $output): void
    {
        ...
        $commands = [
            'git init -q',
            'git add .',
            'git commit -q -m "Set up a fresh Neos project"',
            "git branch -M {$branch}",
        ];
    ...
    protected function verifyApplicationDoesntExist(string $directory): void
    {
        if ((is_dir($directory) || is_file($directory)) && $directory !== getcwd()) {
            throw new RuntimeException('Neos project already exists!');
        }
    }
